...
const CategoryEditPage = () => {
  ...
  useEffect(() => {
    if (categoryUpdated?.id && success) {
      ...
      if (!categoryFetched?.name || !categoryFetched?.id) {
        ...
        dispatch(fetchCategoryDetails(params.subCatId));
      } else {
        setFormData({
  ...
  }, [dispatch, params, success, navigate, categoryFetched, categoryUpdated]);
  ...
  const handleSubmit = (event) => {
    ...
    dispatch(updateCategory(params.subCatId, formData));
  };

  console.log('Category: Name ', params, formData);
  ...
  return (
    <CardContainer>
      ...
      <Box className={'box-spacing'}>
        <Grid container>
          <Grid item xs={3}>
            <Typography sx={{ mt: 1 }}>Sub-Category Name</Typography>
          </Grid>
          <Grid item xs={9}>
            <TextField
              value={formData.name}
